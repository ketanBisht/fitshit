...
let MailService = MailService_1 = class MailService {
    ...
    async sendWelcomeEmail(to, name, role, plainTextPassword, gymName) {
    ...
    async sendPaymentReceiptEmail(to, name, amount, planName, gymName) {
        const transporter = await this.getTransporter();
        if (!transporter)
            return;
    ...
    async sendAnnouncementEmails(gymName, content, emails) {
    ...
    async sendPasswordResetEmail(to, name, token, gymName, subdomain) {
        const transporter = await this.getTransporter();
        if (!transporter)
            return;
